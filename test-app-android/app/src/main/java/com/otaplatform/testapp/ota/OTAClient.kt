    ...
    suspend fun checkForUpdate(): UpdateInfo = withContext(Dispatchers.IO) {
        try {
            ...
            val request = Request.Builder()
                .url("${config.apiBaseUrl}/api/v1/ota/check-update")
                .post(
                    gson.toJson(
                        mapOf(
                            "bundleId" to config.bundleId,
                            "platform" to config.platform,
                            "currentVersion" to config.currentVersion,
            ...
            val body = response.body?.string()
                ?: throw OTAException("Empty response from server")
            ...
            val data = checkUpdateResponse.data
                ?: throw OTAException("No data in response")
            ...
            throw if (e is OTAException) e else OTAException("Failed to check for update", e)
        }
    }
    ...
    suspend fun downloadUpdate(
        ...
        try {
            ...
            client.newCall(request).execute().use { response ->
                ...
                val urlExtension = updateInfo.downloadUrl.substringAfterLast('.', "bundle")
                val fileName = "update_${updateInfo.latestVersion}.$urlExtension"
